...
def evaluate_and_promote(sport: str, kind: str = "winner") -> dict:
    ...
    from src.models.registry import get_run_metrics, promote_model, load_model
    import mlflow
    ...
    with sync_session_factory() as session:
        ...
        champion = session.query(ModelRecord).filter_by(
            sport_id=sport_obj.id, kind=kind, target="home_won", active=True
        ).first()

        champion_metrics = dict(champion.metrics) if champion else {"logloss": 999.0, "ece": 999.0, "brier": 999.0}
        ...
        if promote:
            ...
            session.commit()
            _notify_ops(f"✅ {sport.upper()} {kind} model promoted. LogLoss: {challenger_metrics.get('logloss', '?'):.4f}")
...
def run_drift_monitor(sport: str) -> dict:
    ...
    from src.models.eval.metrics import compute_ece, compute_psi
    from src.core.config import settings
    import numpy as np
    ...
    with sync_session_factory() as session:
        from src.db.models import ModelRecord, Sport as SportModel, Prediction, Game
        sport_obj = session.query(SportModel).filter_by(code=sport).first()
        if sport_obj is None:
            return {"events": []}

        champion = session.query(ModelRecord).filter_by(
            sport_id=sport_obj.id, kind="winner", active=True
        ).first()
        if champion is None:
            return {"events": []}
        ...
        if len(rows) >= 10:
            ...
            if recent_ll > baseline_ll * (1 + settings.drift_logloss_threshold):
                ...
                _notify_ops(f"🔴 Drift: {sport.upper()} logloss {recent_ll:.4f} vs baseline {baseline_ll:.4f}")
                # Trigger priority retrain
                train_challenger.apply_async(kwargs={"sport": sport, "kind": "winner"}, queue="high_priority")
...
train_challenger = app.task(name="src.tasks.train_tasks.train_challenger", bind=False)(train_challenger)
evaluate_and_promote = app.task(name="src.tasks.train_tasks.evaluate_and_promote", bind=False)(evaluate_and_promote)
run_drift_monitor = app.task(name="src.tasks.train_tasks.run_drift_monitor", bind=False)(run_drift_monitor)
generate_backtest_report = app.task(name="src.tasks.train_tasks.generate_backtest_report", bind=False)(generate_backtest_report)
hyperparam_search = app.task(name="src.tasks.train_tasks.hyperparam_search", bind=False)(hyperparam_search)
